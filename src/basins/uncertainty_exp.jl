...
"""
    basins_fractal_dimension(basins; kwargs...) -> ε, N_ε ,d
Estimate the Haussdorff dimension `d` of the basins of attraction using the box-counting
algorithm.
...
The ouput `d` is the estimation of the box-counting dimension of the boundary by
fitting a line in the `log.(N_ε)` vs `log.(1/ε)` curve.
...
function uncertainty_exponent(basins::AbstractArray; range_ε = 2:maximum(size(basins))÷20)
    ε, N_ε, d = basins_fractal_dimension(basins; range_ε)
    return ε, N_ε, length(size(basins)) - d
end

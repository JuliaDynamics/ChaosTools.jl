...
This function returns a number that matches the initial condition `u0` to an attractor.
...
The keywords arguments are the same as defined in the function [`basins_of_attraction`](@ref)
for the control of the basins estimation: `mx_chk_att`, `mx_chk_hit_bas`, `mx_chk_fnd_att`,
`mx_chk_loc_att`, `mx_chk_lost`, and `horizon_limit = 1e6`
"""
